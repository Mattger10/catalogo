...
const Recommended: React.FunctionComponent<RecommendedProps> = ({
  mostrarTabla, handleSelectSong
}) => {
  const allSongs = recommendedData.find((songs) => songs.songs)
  const songs = allSongs ? allSongs.songs : [];
  const [seleccionarCancion, setSeleccionarCancion] = React.useState<string | null>(null);
...
const handleShareOnWhatsApp = (song: string) => {
  ...
  if (urlToShare) {
    ...
    const whatsappShareUrl = `https://wa.me/?text=${encodeURIComponent(textToShare)}`;
    ...
    <div>
      {mostrarTabla ? (
        <Container>
            ...
            <Table sx={{ minWidth: 650 }} aria-label="simple table">
              <TableHead>
                ...
                <TableRow>
                  ...
                  >
                    <Typography sx={{ marginLeft: "-12rem" }}>ALBUM</Typography>
                  </TableCell>
                  <TableCell
                    align="center"
                    sx={{
                  ...
                  >
                    ...
                    >
                      ...
                      <PlayCircleOutlineIcon
                        ...
                        }}  onClick={() => handlePlayPause(song.song_url, song.songName, index)}
                      />
                    ...
                    >
                     <ShareIcon onClick={() => handleShareOnWhatsApp(song.songName)} sx={{cursor: "pointer", "&:hover": {color: "#ed215e"}}} />
                    </TableCell>
                  </TableRow>
                   ) : null; 
                    })}
          ...
          <ContainerButton>
          {songs.length > 5 ? (
            <StyledButton onClick={() => setVerTodasLasCanciones(!verTodasLasCanciones)} sx={{ marginTop: '1rem'}}>
